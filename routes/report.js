...
router.get("", (req, res, next) => {
  const baseTemplate = `<html>
        ...
        <body>
            ...
            <ul>
                <li>
                    <a href='/report/firstCohort?cohortType=firstCohort'>
                        Download First Cohort Report
                    </a>
                </li>
                <li>
                    <a href='/report/secondCohort?cohortType=secondCohort'>
                        Download Second Cohort Report
                    </a>
                </li>
            </ul>
        </body>
...
router.get("/firstCohort", (req, res, next) => {
  ...
  var cohortType = req.query.cohortType;
  //var cohortType = "firstCohort";
  // const cohortType = request.getHeader('');
  // if(startDate > endDate){
  //     res.status(200).end("<span style='color:red'>Improper date range</span>");
  // }
  // console.log(cohortType);
  var cohortId = getCohortTypeId(cohortType);
  // console.log(cohortId);
  var Airtable = require("airtable");
  var base = new Airtable({
    apiKey: process.env.AIRTABLE_API_KEY
  }).base("appfO9PMTzzFk9466");
  base('Exchange rates').find(cohortId, function (err, record) {
    if (err) {
      console.error(err);
      return;
    }
    USDtoINRConversionRate = record.get('Rs 1 USD = x Rs');
    // console.log("amt",USDtoINRConversionRate);
    base("Projects")
      .select({
      ...
      .eachPage(
        function page(records, fetchNextPage) {
          ...
          records.forEach(function (record) {
            ...
            var inrProjectCost = "";
            var usdProjectCost = record.get("Invoiced Cost");
...
function getCohortTypeId(cohortType) {
  if (cohortType == "firstCohort") {
    return "recMXqWU8Ts4j7zRZ";
  } else if (cohortType == "secondCohort") {
    return "recvK1HBfnlSMgAvR";
  }
}
